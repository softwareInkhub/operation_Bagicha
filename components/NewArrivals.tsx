            ...
            {newProducts.map((product, index) => (
              ...
              >
                ...
                <div className="relative h-32 overflow-hidden">
                  ...
                  {/* Wishlist Button */}
                  <motion.button
                    className="absolute top-6 right-1 w-6 h-6 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center hover:bg-white transition-colors shadow-lg"
                    onClick={() => toggleWishlist(product.id)}
                    whileHover={{ scale: 1.1 }}
                    whileTap={{ scale: 0.9 }}
                  >
                    <Heart 
                      className={`w-3 h-3 ${wishlist.includes(product.id) ? 'fill-red-500 text-red-500' : 'text-gray-600'}`} 
                    />
                  </motion.button>
